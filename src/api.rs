...
impl Source {
    pub fn from_repo(path: impl AsRef<std::path::Path>) -> Result<Self, git2::Error> {
        use git2::Repository;
...
impl Environment {
    pub fn generate_from_current_config() -> Self {
        ...
        if let Some(distribution) = System::name() {
            software.push(VersionInfo {
                name: distribution,
                version: String::from("distribution"),
            });
        }
        if let Some(kernel) = System::kernel_version() {
            software.push(VersionInfo {
                name: kernel,
                version: String::from("kernel"),
            });
        }
        if let Some(os) = System::os_version() {
            software.push(VersionInfo {
                name: os,
                version: String::from("kernel-release"),
            });
        }
        if let Some(arch) = System::cpu_arch() {
            software.push(VersionInfo {
                name: arch,
                version: String::from("arch"),
            });
        }
